...
impl StateMachine for WeirdSwitchMachine {
    ...
    fn next_state(starting_state: &TwoSwitches, t: &Toggle) -> TwoSwitches {
        match t {
            Toggle::FirstSwitch => {
                match starting_state {
                    &TwoSwitches { first_switch: false, second_switch: _ } =>
                        TwoSwitches { first_switch: true, second_switch: starting_state.second_switch },
                    &TwoSwitches { first_switch: true, second_switch: _ } =>
                        TwoSwitches { first_switch: false, second_switch: false },
                }
            },
            Toggle::SecondSwitch => {
                    match starting_state {
                        &TwoSwitches { first_switch: _, second_switch: false } =>
                            TwoSwitches { first_switch: starting_state.first_switch, second_switch: true },
                        &TwoSwitches { first_switch: _, second_switch: true } =>
